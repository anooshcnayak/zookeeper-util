...
class ZkUtil {
  ...
  async getProperty(
    ...
    if (watcher) this.watchers[keyPath] = watcher; // Adding watcher
        ...
        function (event: any) {
          ...
          if (event && event.type === 3 && _this.watchers[keyPath]) {
            ...
            _this
              ...
              .then((data: any) => {
                _this.watchers[keyPath](null, data);
              })
              .catch((err: Error) => {
                _this.watchers[keyPath](err);
              });
          }
        },
  ...
  setProperty(key: string, data: any): Promise<void> {
    ...
    return new Promise<void>((resolve: any, reject: any) => {
      this.client.setData(keyPath, ZkUtil.converDataToUtf8(data), function (
        error: Error
      ) {
        if (error) {
  ...
  static converDataToUtf8(data: any): any {
